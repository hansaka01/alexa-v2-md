...
const reply = (teks) => {
  ZimBotInc.sendMessage(m.chat, {text: teks, contextInfo: {"externalAdReply": {title: "𝙰𝙻𝙴𝚇𝙰-𝚅2",mediaType: 3, renderLargerThumbnail: false, showAdAttribution: true, detectLinks: true,body: "DRIPS OFC", thumbnail: fs.readFileSync('./drips.jpg'),sourceUrl: ("https://youtu.be/KNu-gr2h7bo")}}})
}
const replay = (teks) => {
  ZimBotInc.sendMessage(m.chat, {text: teks, contextInfo: {"externalAdReply": {title: "𝙰𝙻𝙴𝚇𝙰-𝚅2",mediaType: 3, renderLargerThumbnail: false, showAdAttribution: true, body: "DRIPS OFC", thumbnail: fs.readFileSync('./drips.jpg'),sourceUrl: ("https://youtu.be/KNu-gr2h7bo")}}})
}    
...
//     'X-API-KEY': '4052885a-e219-4cf8-a859-10bbceb4c986'
//   },
//   body: JSON.stringify({enable_google_results: 'true', enable_memory: false, input_text: `${command} ${text}`})
// };

//  fetch(url, options)
//   .then(res => res.json())
//   .then(json => replytx = json)
//   .then(json => console.log(json))
//   .catch(err => console.error('error:' + err));
// //  replytx = await fetchJson(json)
// //   replytx1 = `${replytx.message}`
// ini_txt = 
//  m.reply(`${replytx.message}`)
...
var hg = `*Mission to kill Slime*\n\n🎁 *Reward for killing Slime*\n ┊ *Money:* $${b}\n ┊ *Iron:* ${c}\n ┊ *Gold:* ${a}\n ┊ *Diamond:* ${d}\n\n*Thank you for carrying out this mission*`
          ...
          const emote = (satu, dua) => {
            ...
            const buttonMessage = {image: {url: emoji.images[dua].url},caption: "𝙰𝙻𝙴𝚇𝙰-𝚅2",footerText: 'Loading...',buttons: buttons,headerType: 4}
...
openapikey = await heroku.get(Config.OPEN_API_KEY)
...
break
 ...
 case 'public': {
   ...
   reply('Alexa 𝗻𝗼𝘄 𝘄𝗼𝗿𝗸𝗶𝗻𝗴 𝗮𝘀 𝗽𝘂𝗯𝗹𝗶𝗰')
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim(Hansaka)} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
   `.trim()
   reply(respon)
 }
 break
